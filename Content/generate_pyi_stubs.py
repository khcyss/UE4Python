...
def write_ue_classes(file, classes):
    for class_ in classes:
        ...
        localclassProperties = class_.properties()
        #for property_ in filter_names(localclassProperties):
        for property_ in localclassProperties:
            file.write("    ")
            #file.write("{Propertyname}:  {type} \n".format(Propertyname= property_, type= 'bool' if property_.startswith('b') else 'typing.Any'))
            file.write("{} \n".format(property_))
            attributes_count += 1

        localclassFunction = class_.functions()
        #for function_ in filter_names(localclassFunction):
        for function_ in localclassFunction:
            file.write("    ")
            #file.write("def {}(*args, **kwargs) -> 'typing.Any': pass\n".format(function_))
            file.write("def {}".format(function_))
            attributes_count += 1
...
if __name__ == '__main__':
    generate_pyi_stubs(os.path.join(BlueprintPathsLibrary.ProjectIntermediateDir(), 'PythonStub'), True)
